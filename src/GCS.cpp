                                                    ...
                                                    )
{
    ...
    if(debugMode==Minimal || debugMode==IterationLevel) {
        ...
        std::cout << "Sketcher::RedundantSolving-" << solvername.c_str() << "-\n" << std::endl;
    }
    ...
    if (res == Success) {
        ...
        if(debugMode==Minimal || debugMode==IterationLevel) {
            std::cout << "Sketcher Redundant solving: " << redundant.size() << " redundants" << std::endl;
        }
        ...
        for (int i=conflictGroupsOrig.size()-1; i >= 0; i--) {
            ...
            for (std::size_t j=0; j < conflictGroupsOrig[i].size(); j++) {
                if (redundant.count(conflictGroupsOrig[i][j]) > 0) {
                    ...
                    if(debugMode==IterationLevel) {
			    std::cout << "(Partially) Redundant, Group " << i << ", index " << j << ", Tag: " << (conflictGroupsOrig[i][j])->getTag() << std::endl;
                    }
